...
router.get('/login', (req, res) => {
  if (req.session.user) {
    switch (req.session.user.user_type) {
      ...
      case 'officer':
      case 'department_head': return res.redirect('/officer/dashboard');
      case 'admin': return res.redirect('/admin/dashboard');
      default: return res.redirect('/');
    }
  }
  res.render('auth/login', { error: req.query.error || null, message: req.query.message || null });
});
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (!validPassword) return res.redirect('/login?error=Invalid credentials');
    ...
    switch (user.user_type) {
      case 'citizen': res.redirect('/citizen/dashboard'); break;
      case 'officer':
      case 'department_head': res.redirect('/officer/dashboard'); break;
...
router.get('/register', (req, res) => {
  ...
  res.render('auth/register', { error: req.query.error || null, message: req.query.message || null });
});
